...
import { addToCart, getCart, removeFromCartItem } from "../features/products/productSlice";
...
const Cart = () => {
  ...
  const handleQuantityChange = async (size_color_quantity_id, quantity, product_total) => {
    const item = cartItems.find(item => item.size_color_quantity_id === size_color_quantity_id);
    const availableQuantity = item.productDetails.size_color_quantity;
  
    if (quantity > availableQuantity) {
      // Show an error message to the user
      alert(`Only Rs {availableQuantity} items are available for this product.`);
      return;
    }
              ...
              {cart && cart?.map((item, index) => (
                <div key={index} className="cart-data py-3 mb-2 d-flex justify-content-between align-items-center">
                  ...
                  <div className="cart-col-2">
                    <h5 className="price">Rs {item?.productDetails?.unit_price}</h5>
                  </div>
                  <div className="cart-col-3 d-flex align-items-center gap-15">
                    ...
                    ( <input
                     ...
                     value={item?.quantity}
                     onChange={(e) => handleQuantityChange(item?.size_color_quantity_id, parseInt(e.target.value), (Number(item?.productDetails?.unit_price) * parseInt(e.target.value)) )}
                   ...
                   ( <input
                    ...
                    <div>
                      <AiFillDelete onClick={() => handleRemoveFromCartItem(item?.cart_item_id)} className="text-danger " />
                    </div>
                  </div>
                  <div className="cart-col-4">
                   
                  <h5 className="price">Rs {(Number(item?.productDetails?.unit_price) * item?.quantity).toFixed(2)}</h5>
                  </div>
                </div>
              ))}
            </div>
